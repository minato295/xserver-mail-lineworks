...
`provider_description`はLINE WORKS応答由来であり、信頼済みのローカル文言として扱いません。producerとloggerの両方で型・長さ・制御文字・応答形式との相関を検証し、説明の前後にprovider固有文言が付いていても、送信した件名または本文との間に8 bytes以上の共通連続断片があれば説明を保存しません。判定は固定長window集合と各文字列1回の走査で行い、大きなpayloadに対して説明断片ごとの全payload再走査を行いません。Mac側も新規ログのC0・DEL・C1制御文字を拒否し、表示直前に再度除去・秘密値判定を行います。メール内容を保存しない契約はこの境界でも維持します。
